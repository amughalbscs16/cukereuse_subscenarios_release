...
> Mughal, A. H., Fatima, N., & Bilal, M. (2026). *Given, When, Then, Again: Mining Subscenario Refactoring Candidates in Behaviour-Driven Test Suites with ML Classifiers and LLM-Judge Baselines.* arXiv preprint [arXiv:2605.14568](https://arxiv.org/abs/2605.14568). (The arXiv v1 was announced under the previous title *Mining Subscenario Refactoring Opportunities in Behaviour-Driven Software Test Suites: ML Classifiers and LLM-Judge Baselines*; a v2 carrying the current title is pending.) A revised version has been prepared for submission to the *Journal of Systems and Software* (Elsevier).
...
| `reusable_scenario_report.html` | n/a | self-contained HTML report of the top reusable-scenario candidates |
| `corpus_summary.json` | n/a | one-stop corpus shape (339 repos, 276 orgs, 126,621 scenarios, 1,113,616 steps) |
| `rule_baseline_metrics.json` | n/a | rule baseline + XGBoost OOF + McNemar (vs. R1, vs. each LLM judge) |
| `extraction_classifier_metrics.json` | n/a | per-fold and bootstrap-CI metrics for XGBoost extraction-worthy classifier |
| `mechanism_classifier_metrics.json` | n/a | per-fold and bootstrap-CI metrics for XGBoost mechanism classifier |
| `post_classifier_headline.json` | n/a | scenario- and repo-level extraction-worthy headline rates (RQ1/RQ2/RQ3) |
| `preliminary_rollups.json` | n/a | preliminary headline numbers (pre-classifier sanity rollup) |
| `phase2c_summary.json` | n/a | exact-subsequence count summary (counts of distinct patterns by scope) |
| `slices_summary.json` | n/a | slice inventory summary (per-L bucket counts) |
| `slice_clusters_summary.json` | n/a | cluster-density summary |
| `exact_subsequence_summary.json` | n/a | recurring-pattern summary |
| `extraction_classifier_feature_importance.json` | n/a | XGBoost gain-based importances |
| `scenario_identity_audit.json` | n/a | scenario-name uniqueness audit |
| `scenario_length_distribution.json` | n/a | distribution of scenario lengths (steps per named scenario) |
| `scenario_length_histogram.csv` | n/a | tabular scenario-length histogram |
| `spec_suite_outliers.csv` | n/a | repos flagged as auto-generated spec suites (excluded from headline rates) |
...
- **Author-labelled extraction-worthy rate (200-slice stratified pool):** 71.5% yes, 13.0% no, 11.0% flagged-spec, 3.0% uncertain, 1.5% tie.
...
Please cite both the paper (arXiv preprint below) and this software/data archive. Once a journal version of record is published, the citation will be updated to point to it. The `CITATION.cff` file is rendered by GitHub's "Cite this repository" widget; its `preferred-citation` field tracks the latest citable version.

### BibTeX

```bibtex
@misc{mughal2026givenwhenthenagain,
  ...
  note          = {arXiv v1 announced under the previous title; revised version
                   prepared for submission to the \emph{Journal of Systems and
                   Software} (Elsevier). Citation will be updated to the journal
                   version on acceptance.}
}
